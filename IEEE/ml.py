...
def eeg_analysis():
    ...
    # EEG electrode placement as key, physical position on Enophones as item.
    channel_to_position = {'A2': 'Right Cushion', 'A1': 'Left Cushion',
                           'C4': 'Top Right', 'C3': 'Top Left'}
        ...
        for df_name in os.listdir('PruebasConBeeps/{}/Raw/'.format(folder)):
            ...
            if df_name[-4:] == '.csv' and df_name[:4] != 'file' and len(df_name.split('_')) == 1:
                ...
                if n in [323, 325]:
                    ...
                    df_eeg_temp.columns = [
                        col.split('_')[0] + '_' + list(channel_to_position.keys())[int(col.split('_')[1][1]) - 1] for
                        col in df_eeg_temp.columns]
...
def comb_features():
    ...
    def combined_features(df, information_features_combined):
        ...
        for i in range(len(df.columns)):
            ...
            df_combined = pd.concat((df_combined, pd.Series(np.log(np.abs(np.array(df.loc[:, df.columns[i]])) + 1))),
                                    axis=1, ignore_index=True)
    ...
    for subject in df_feat.Subject.unique():
        ...
        if empatica == False and df_processed.shape[0] > 10:
...
number_features = {'empatica': 10,
                   'non-empatica': 10}
create_model(tuned=False, mean_plot=False)
