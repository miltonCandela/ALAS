...
def get_fatigue():
    ...
    ids_to_name = pd.read_csv('Empatica-Project-ALAS-main/Files/ids.txt', sep='\t', header=None).drop(1, axis=1)
...
def eeg_analysis():
    ...
    spectral_signals = {'Alpha': [8, 12], 'Beta': [13, 29],
                        'Gamma': [30, 50], 'Theta': [4, 7], 'Delta': [1, 4]}
    ...
    def get_emp(folder_name, eeg_index):
        df_ecg = pd.DataFrame()
        for df_name in ['fileBVP', 'fileEDA', 'fileTemp']:

            df = pd.read_csv('PruebasConBeeps/{}/Raw/{}.csv'.format(folder_name, df_name))
            ...
            df['Bins'] = pd.cut(df['Second'], bins=[0] + list(eeg_index), duplicates='drop', include_lowest=True)
            ...
            # name declared before in the list "func_name".
                ...
                df.columns = ['Tem_' + func_name for func_name in ['Mean']]
            ...
            df_ecg = pd.concat([df_ecg, df], axis=1)

        return df_ecg

    for folder in os.listdir('PruebasConBeeps/'):
        df_raw = (pd.read_csv('PruebasConBeeps/{}/Raw/Crudas.csv'.format(folder), index_col=0)
        ...
        for n in range(df_raw.shape[0] // 256):
            ...
            # reference = np.sum(reference, axis=1)/2

            # df_unpross[:, 2] = df_unpross[:, 2] - ((df_unpross[:, 0] + df_unpross[:, 1]) / 2)  # C3 - ((A1 + A2)/2)
            # df_unpross[:, 3] = df_unpross[:, 3] - ((df_unpross[:, 0] + df_unpross[:, 1]) / 2)  # C4 - ((A1 + A2)/2)
            ...
            for n_channel in range(n_channels):
                ...
                # 60 Hz Notch filter
                DataFilter.perform_bandstop(data=df_unpross, sampling_rate=sr, start_freq=59,
                                            stop_freq=61, order=eta, filter_type=ft, ripple=0)

                # 0.1–100 Hz 4th order Butterworth bandpass filter
                DataFilter.perform_lowpass(data=df_unpross, sampling_rate=sr, cutoff=100,
                                           order=eta, filter_type=ft, ripple=0)
                DataFilter.perform_highpass(data=df_unpross, sampling_rate=sr, cutoff=0.1,
                                            order=eta, filter_type=ft, ripple=0)
        ...
        n = df_eeg.shape[0]
        if n in [321, 322, 323]:
            df_eeg['Subject'], df_eeg['Repetition'], df_eeg['Second'] = folder[1:3], folder[4:6], range(325 - n, 325)

            if empatica and folder[1:3] != '99':
                df_emp = get_emp(folder, range(325-n, 325))
                df_emp['Second'] = list(range(325-n, 325))
...
def comb_features():
    ...
    for subject in df_feat.Subject.unique():
        ...
        df_processed = normalize(combined_features(remove_outliers(df_filtered, norm_method), ['Second']),
                                 scaler_method)
        ...
        if df_processed.shape[0] > n_valid:
            df_total = pd.concat([df_total, df_processed], ignore_index=True)

    exit()
    df_total.to_csv('Paper_Data/{}.csv'.format(file_name), index=False)
    exit()
...
n_valid = 100
file_name = 'EEG{}_{}{}{}{}{}{}N{}_R'.format(ecg_name, norm_name,
